...
from sc2.constants import NEXUS, PROBE, PYLON, ASSIMILATOR, GATEWAY, CYBERNETICSCORE, STARGATE, VOIDRAY, OBSERVER, ROBOTICSFACILITY
import random
import cv2
import numpy as np
import time
...
class HuyAI(sc2.BotAI):
    ...
    async def scout(self):
        if len(self.units(OBSERVER)) > 0:
            ...
            for rf in self.units(ROBOTICSFACILITY).ready.noqueue:
                if self.can_afford(OBSERVER) and not self.already_pending(OBSERVER):
    ...
    async def attack(self):
        if len(self.units(VOIDRAY).idle) > 0:
            current_state = self.flipped.reshape(1, 176, 200, 3)
            choice = self.model.predict_classes(current_state)[0]
            print(choice)
            target = False
    ...
    async def intel(self):
        ...
        self.flipped = cv2.flip(game_data, 0)  # flipping because cv2 counts from top supply_left
        if not HEADLESS:
            resized = cv2.resize(self.flipped, dsize=None, fx=2, fy=2)  # Largen the image
